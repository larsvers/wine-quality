...
function drawBlackBox(ctx, t) {
  console.log(state.blackBox.boxDims.width);
...
function drawLolliProps(ctx, t, input, output) {
  ctx.clearRect(0, 0, state.width, state.height);
  ctx.save();
...
function renderBlackBox() {
  requestAnimationFrame(() => {
    ...
    drawLolliProps(state.ctx.lolli, state.transform.bottle, inputProperties, [
      'quality',
    ]);
  });
}
